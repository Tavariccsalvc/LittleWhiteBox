...
class VariablesPanel {
  ...
  installMessageButtons(){
    ...
    const addH=(type,fn)=>{ if(event_types[type]) eventSource.on(event_types[type],fn); };
    this.eventHandlers.messageRendered = (d)=> delayedAdd(idFrom(d));
    this.eventHandlers.messageReceived = (d)=> delayedAdd(idFrom(d));
    this.eventHandlers.messageUpdated  = (d)=> delayedAdd(idFrom(d));
    this.eventHandlers.messageSwiped   = (d)=> delayedAdd(idFrom(d));
    this.eventHandlers.messageEdited   = (d)=> delayedAdd(idFrom(d));
    this.eventHandlers.messageSent     = debounce(()=> delayedScan(),300);
    this.eventHandlers.chatChanged     = ()=> delayedScan();
    if(eventSource){
      addH('USER_MESSAGE_RENDERED', this.eventHandlers.messageRendered);
      addH('CHARACTER_MESSAGE_RENDERED', this.eventHandlers.messageRendered);
      addH('MESSAGE_RECEIVED', this.eventHandlers.messageReceived);
      addH('MESSAGE_UPDATED', this.eventHandlers.messageUpdated);
      addH('MESSAGE_SWIPED', this.eventHandlers.messageSwiped);
      addH('MESSAGE_EDITED', this.eventHandlers.messageEdited);
      addH('MESSAGE_SENT', this.eventHandlers.messageSent);
      addH('CHAT_CHANGED', this.eventHandlers.chatChanged);
    }
  ...
  removeMessageButtonsListeners(){
    ...
    const rm=(type,fn)=>{ if(fn && event_types[type]) eventSource.removeListener(event_types[type],fn); };
    rm('USER_MESSAGE_RENDERED',h.messageRendered);
    rm('CHARACTER_MESSAGE_RENDERED',h.messageRendered);
    rm('MESSAGE_RECEIVED',h.messageReceived);
    rm('MESSAGE_UPDATED',h.messageUpdated);
    rm('MESSAGE_SWIPED',h.messageSwiped);
    rm('MESSAGE_EDITED',h.messageEdited);
    rm('MESSAGE_SENT',h.messageSent);
    rm('CHAT_CHANGED',h.chatChanged);
    this.eventHandlers={};
  }
