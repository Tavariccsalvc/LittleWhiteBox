...
function bindMessageEvents() {
 ...
 state.eventHandlers = {
   ...state.eventHandlers,
   onReceived: () => { refreshOnAI(); },
   onGenEnd: () => { refreshOnAI(); },
   onSent: () => {},
   onDeleted: () => {},
   onUpdated: () => { refreshOnAI(); },
   onSwiped: () => { refreshOnAI(); },
   onGenStart: () => {}
 };
...
function unbindMessageEvents() {
 ...
 const events = [
   [event_types.MESSAGE_SENT, 'onSent'],
   [event_types.MESSAGE_RECEIVED, 'onReceived'],
   [event_types.MESSAGE_DELETED, 'onDeleted'],
   [event_types.MESSAGE_UPDATED, 'onUpdated'],
   [event_types.MESSAGE_SWIPED, 'onSwiped'],
   [event_types.GENERATION_ENDED, 'onGenEnd']
 ];

 if (event_types.GENERATION_STARTED) {
   events.push([event_types.GENERATION_STARTED, 'onGenStart']);
 }

 events.forEach(([eventType, handlerName]) => {
   eventSource.off(eventType, state.eventHandlers[handlerName]);
 });
...
function cleanup() {
 ...
 if (typeof eventSource !== 'undefined' && state.eventHandlers.chatChanged) {
